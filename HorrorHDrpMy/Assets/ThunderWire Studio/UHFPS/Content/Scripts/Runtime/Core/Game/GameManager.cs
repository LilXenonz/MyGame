...
namespace UHFPS.Runtime
{
    ...
    public class GameManager : Singleton<GameManager>
    {
        ...
        public CustomPassVolume BlurPass;
        ...
        /// <summary>
        /// Freeze player controls except.
        /// </summary>
        public void FreezePlayerExcept(bool state, Type[] components, bool showCursor = false, bool lockInput = true)
        {
            PlayerPresence.FreezePlayerExcept(state, components, showCursor);
            isInputLocked = lockInput && state;
        }

        /// <summary>
        /// Freeze player controls except Motion Controller.
        /// </summary>
        public void FreezePlayerExceptMotion(bool state, bool showCursor = false, bool lockInput = true)
        {
            PlayerPresence.FreezePlayerExcept(state, new[] { typeof(MotionController) }, showCursor);
            isInputLocked = lockInput && state;
        }
        ...
        {
            if (GlobalPPVolume != null && GlobalPPVolume.profile.TryGet(out T component))
            {
        ...
        public void SetBlur(bool active, bool interpolate = false)
        {
            ...
            if (!interpolate)
            {
                if (BlurPass.customPasses[0] is not GaussianBlur blur) 
                    return;
            ...
            {
                InterpolateBlur(1f, BlurDuration);
            }
            else
            {
        ...
        /// </summary>
        public void InterpolateBlur(float targetWeight, float duration)
        {
            if (!EnableBlur) 
                return;
            ...
            if (targetWeight > 0) blur.BlurWeight = 0f;
            blur.enabled = true;

            if (blurCoroutine != null) blurCoroutine.Stop();
            blurCoroutine = CoroutineRunner.Run(gameObject, InterpolateBlur(blur, targetWeight, duration));
        }

        IEnumerator InterpolateBlur(GaussianBlur blur, float targetWeight, float duration)
        {
            float startBlurRadius = blur.BlurWeight;
            float elapsedTime = 0;

            while (elapsedTime < duration)
            {
                elapsedTime += Time.deltaTime;
                blur.BlurWeight = Mathf.Lerp(startBlurRadius, targetWeight, elapsedTime / duration);
                yield return null;
            }

            blur.BlurWeight = targetWeight;
            if(targetWeight <= 0) blur.enabled = false;
        }
        ...
        public void ShowInventoryPanel(bool state)
        {
            if (PlayerPresence.IsUnlockedAndCamera && !PlayerDied && !isInputLocked && !IsPaused)
            {
                ...
                // show blur
                if (IsInventoryShown) InterpolateBlur(1f, BlurDuration);
        ...
        private void OnPause(InputAction.CallbackContext obj)
        {
            if (obj.ReadValueAsButton() && PlayerPresence.IsUnlockedAndCamera && !PlayerDied && !isInputLocked && !IsInventoryShown)
            {
                ...
                // show blur
                if (IsPaused) InterpolateBlur(1f, BlurDuration);
                else InterpolateBlur(0, BlurDuration);
