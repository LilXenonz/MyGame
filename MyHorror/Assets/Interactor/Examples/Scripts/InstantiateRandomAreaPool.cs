...
namespace razz
{
	...
	public class InstantiateRandomAreaPool : MonoBehaviour
	{
		...
		[Tooltip("When objects spawned with a parent on start, it sets parent object to InteractorObject's OtherTargetsRoot so InteractorObject will get targets from there.")]
		public bool setSpawnedParentToIntObjRoot = false;
		...
		private void Awake()
		{
            ...
            if (setSpawnedParentToIntObjRoot)
            {
				intObj.otherTargetsRoot = _pool;
				intObj.GetChildTargets();
			}
		...
		private void Update()
		{
			...
			if (InteractorInput.GetKeyDown(KeyCode.Return))
		...
		private void InstantiatePool()
		{
			...
			for (int i = 0; i < maxPrefabCount; i++)
			{
				...
				_prefabList.Add(GameObject.Instantiate(prefab, GetPoint(), prefab.transform.rotation, _pool.transform));
