...
namespace razz
{
    ...
    {
        public string useKeyText = "Press E to Use";
        public string clickKeyText = "Left Click to Use";
        ...
        private void Update()
        {
            ...
            float mouseWheel = InteractorInput.GetMouseWheel();
        ...
        private void ShowIntObjList()
        {
            ...
            while (i <= _interactor.intObjComponents.Count)
            {
                ...
                else
                {
                    ...
                    if (type == InteractionTypes.DefaultOnce || type == InteractionTypes.SelfPathMover || type == InteractionTypes.TouchHorizontalUp || type == InteractionTypes.TouchStill || type == InteractionTypes.TouchVertical || type == InteractionTypes.DistanceRange)
                    {
                        i++;
                        continue;
                    }
                    ...
                    if (_interactor.intObjComponents[i].interactorObject.ready && i == _interactor.selectedByUI)
                    {
                        ...
                        {
                            GUI.Label(new Rect(Screen.width / 2 - 40, Screen.height / 3 * 2, 150, 50), clickKeyText, _label);
                        }
                        else
                        {
                            GUI.Label(new Rect(Screen.width / 2 - 40, Screen.height / 3 * 2, 150, 50), useKeyText, _label);
                        }
                    }
                }
                i++;
