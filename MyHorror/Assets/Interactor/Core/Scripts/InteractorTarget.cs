...
namespace razz
{
    ...
    public class InteractorTarget : MonoBehaviour
    {
        ...
        public float pinTargetWhenDone = 0f;
...
#region InteractorPath
        ...
        {
            if (pinTargetWhenDone > 0f)
            {//returns cached last position when back path starts to ignore target movement
                if (weight == 1f)
                {
                    Vector3 currentTargetPos = GetPosition(weight, false);
                    _lastTargetPosition = intObj.currentInteractor.playerTransform.InverseTransformPoint(currentTargetPos);
                    return currentTargetPos;
                }
                else
                {
                    Vector3 currentTargetPos = GetPosition(weight, false);
                    return Vector3.Lerp(_bonePositionBeforeIK, Vector3.Lerp(currentTargetPos, intObj.currentInteractor.playerTransform.TransformPoint(_lastTargetPosition), pinTargetWhenDone), weight);
                }
            }
        ...
        public Quaternion GetRotation(Quaternion rot, float weight, bool halfDone)
        {
            ...
            if (pinTargetWhenDone > 0f)
            {
                if (!halfDone)
                {
                    _lastTargetRotation = Quaternion.Inverse(intObj.currentInteractor.playerTransform.rotation) * this.transform.rotation;
                ...
                {
                    if (weight == 1f)
                    {//if (!halfDone || weight == 1f) doesnt work well it seems
                        _lastTargetRotation = Quaternion.Inverse(intObj.currentInteractor.playerTransform.rotation) * this.transform.rotation;
                        rot = Quaternion.Lerp(rot, this.transform.rotation, weight);
                        return GetRotationOffset() * rot;
                    }

                    rot = Quaternion.Lerp(rot, Quaternion.Lerp(this.transform.rotation, intObj.currentInteractor.playerTransform.rotation * _lastTargetRotation, pinTargetWhenDone), weight);
                    return GetRotationOffset() * rot;
                }
            }
            else
        ...
        { //Increase elapsed time smoothly if backPath gets longer and longer
            if (pinTargetWhenDone > 0f) return 1f;
