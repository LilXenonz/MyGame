using UnityEngine;
using UnityEditor;
using EditorUtil = razz.FullBodyIKEditorUtility;
...
namespace razz
{
	public class FullBodyIKInspectorBase : Editor
	{
		...
		static readonly string[] toolbarContents = new string[] { "Effector", "Bone" };
		//static readonly string[] toolbarContents = new string[] { "Effector", "Bone", "Advanced" }; //For advanced internal settings
		...
		void _EffectorField( string effectorName, ref FullBodyIK.Effector effector )
		{
			...
			if( editorSettings.isShowEffectorTransform ) {
				...
				EditorUtil.GUI.HorizonalSlider( fbik, ref effector.positionWeight, 0.0f, 1.0f, GUILayout.ExpandWidth( false ), GUILayout.Width( 50.0f ) );
				EditorUtil.GUI.PushEnabled( effector.pullContained );
				EditorGUILayout.LabelField( "Pull", GUILayout.Width( labelSpace ) );
				EditorUtil.GUI.HorizonalSlider( fbik, ref effector.pull, 0.0f, 1.0f, GUILayout.ExpandWidth( false ), GUILayout.MinWidth( 50.0f ) );
				EditorUtil.GUI.PopEnabled();
				EditorUtil.GUI.PopEnabled();
			...
			/*EditorGUILayout.LabelField("pos: " + effector.positionWeight + " " + "pull: " + effector.pull + " " + "rot: " + effector.rotationWeight);*/
		}
		...
		public override void OnInspectorGUI()
		{
			...
			if (!fbik.fullBodyIK.editorSettings.debugMode)
			{
				GUILayout.Label("", _guiStyle_header, GUILayout.Height(18.0f));
				GUILayout.BeginHorizontal();
				GUILayout.FlexibleSpace();
				GUILayout.Label("Fully Automated", _guiStyle_header, GUILayout.Height(18.0f));
				GUILayout.FlexibleSpace();
				GUILayout.EndHorizontal();
			}
			...
			GUILayout.BeginHorizontal();
			GUILayout.FlexibleSpace();
			EditorUtil.GUI.Toggle(fbik, "Debug Mode", ref editorSettings.debugMode);
			GUILayout.FlexibleSpace();
			GUILayout.EndHorizontal();

			if (!fbik.fullBodyIK.editorSettings.debugMode) return;
			...
			{
				case 0: _OnInspectorGUI_Effectors();
						break;
				case 1: _OnInspectorGUI_Bones();
						break;
				case 2: _OnInspectorGUI_Advanced();
						break;
			}
		}

		void _OnInspectorGUI_Advanced()
		{
			var fbik = this.target as FullBodyIKBehaviourBase;
		...
		void _OnInspectorGUI_Bones()
		{
			...
			for (int i = 0; i < 2; ++i)
			{
				...
				_BoneField(prefix + "ArmRoll", ref armBones.armRoll, true);
				_BoneField(prefix + "HandRoll", ref armBones.handRoll, true);
				_BoneField(prefix + "Elbow", ref armBones.elbow, false);
				_BoneField(prefix + "ElbowPole", ref armBones.elbowPole, true);
		...
		void _OnInspectorGUI_Effectors()
		{
			...
			EditorGUILayout.Separator();
			_Header("Roll Bones");
			EditorUtil.GUI.Toggle(fbik, "Roll Bones Enabled", ref fbik.fullBodyIK.settings.rollEnabled);

			if (fbik.fullBodyIK.settings.rollEnabled)
			{
				for (int i = 0; i < 2; ++i)
				{
					var effectors = (i == 0) ? leftArmEffectors : rightArmEffectors;
					var prefix = (i == 0) ? "L " : "R ";
					EditorUtil.GUI.Slider(fbik, prefix + "Elbow Pole Horizontal", ref effectors.elbowPoleHorizontal, 0.0f, 1.0f);
					EditorUtil.GUI.Slider(fbik, prefix + "Elbow Pole Vertical", ref effectors.elbowPoleVertical, 0.0f, 1.0f);
				}
			}
	...
	[CustomEditor( typeof( razz.FullBodyIKBehaviour ) )]
