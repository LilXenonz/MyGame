...
namespace razz
{
	public partial class FullBodyIK
	{
		...
		public class Bone
		{
			...
			public FastLength _defaultLocalLength = new FastLength();
			...
			[SerializeField]
			bool _isWritebackWorldPosition = false;
			...
			public void Prepare( FullBodyIK fullBodyIK )
			{
				...
				if( this.transformIsAlive ) {
					...
					FBIKMatSetRot( out _defaultBasis, ref _defaultRotation );
					if( _parentBone != null ) {
					...
					FBIKMatMultInv0( out _worldToBaseBasis, ref _defaultBasis, ref fullBodyIK.internalValues.defaultRootBasis );
					_baseToWorldBasis = _worldToBaseBasis.transpose;
					FBIKMatGetRot( out _worldToBaseRotation, ref _worldToBaseBasis );
				...
				if( this.transformIsAlive && (_parentBone != null && _parentBone.transformIsAlive) ) {
					if( _localAxisFrom == _LocalAxisFrom.Parent ||
						...
						if( dir.x != 0.0f || dir.y != 0.0f || dir.z != 0.0f ) {
							if( _localAxisFrom == _LocalAxisFrom.Parent ) {
								FBIKComputeBasisFrom( out _localAxisBasis, ref fullBodyIK.internalValues.defaultRootBasis, ref dir, _localDirectionAs );
							}
							...
							if( _parentBone._localAxisFrom == _LocalAxisFrom.Child ) {
								if( _parentBone._boneType == BoneType.Shoulder ) {
									...
									if( fullBodyIK.internalValues.shoulderDirYAsNeck == -1 ) {
										if( fullBodyIK.settings.shoulderDirYAsNeck == AutomaticBool.Auto ) {
											if( spineUBone != null && neckBone != null ) {
												...
												if( FBIKVecNormalize( ref shoulderToNeck ) ) {
									...
									if( FBIKVecNormalize2( ref yDir, ref zDir ) ) {
									...
									if( _parentBone._boneType == BoneType.Hips ) {
										...
										FBIKComputeBasisFromXYLockY( out _parentBone._localAxisBasis, ref baseX, ref dir );
										...
										if( _parentBone._parentBone != null ) {
											...
											FBIKComputeBasisFromXYLockY( out _parentBone._localAxisBasis, ref dirX, ref dir );
										}
									} else {
										if( _localAxisFrom == _LocalAxisFrom.Parent && _localDirectionAs == _parentBone._localDirectionAs ) {
											...
											FBIKComputeBasisFrom( out _parentBone._localAxisBasis,
			...
			public void PostPrepare()
			{
				if( _localAxisFrom != _LocalAxisFrom.None ) {
					...
					FBIKMatGetRot( out _localAxisRotation, ref _localAxisBasis );
					_localAxisRotationInv = Inverse( _localAxisRotation );
					FBIKMatMultInv0( out _worldToBoneBasis, ref _defaultBasis, ref _localAxisBasis );
					_boneToWorldBasis = _worldToBoneBasis.transpose;
					FBIKMatGetRot( out _worldToBoneRotation, ref _worldToBoneBasis );
				...
				FBIKMatMultInv0( out _baseToBoneBasis, ref _worldToBaseBasis, ref _worldToBoneBasis );
			...
			public void SyncDisplacement()
			{
				if( _parentBone != null && _parentBone.transformIsAlive && transformIsAlive ) {
					...
					if( _parentBone.transform == this.transform.parent ) {
						...
						if( FBIKVecNormalize( ref localPosition ) ) {
							Vector3 tempDirection;
							FBIKMatMultVec( out tempDirection, ref _parentBone._defaultBasis, ref localPosition );
			...
			public void forcefix_worldRotation()
			{
				if( this.transformIsAlive ) {
					...
					if( _parentBone != null && _parentBone.transformIsAlive ) {
						...
						FBIKMatSetRotMultInv1( out parentRotationBasis, ref parentWorldRotation, ref this.parentBone._defaultRotation );

						Vector3 parentWorldPosition = parentBone.worldPosition;

						Vector3 tempPos;
						FBIKMatMultVecPreSubAdd( out tempPos, ref parentRotationBasis, ref _defaultPosition, ref parentBone._defaultPosition, ref parentWorldPosition );
