...
namespace razz
{
	public partial class FullBodyIK
	{
		public class BodyIK
		{
			...
			Bone[] _spineBones;
			bool[] _spineEnabled;
			Matrix3x3[] _spinePrevCenterArmToChildBasis;
			Matrix3x3[] _spineCenterArmToChildBasis;
			...
			float _defaultCenterLegLen;
			float _defaultCenterLegHalfLen;
			float _defaultNearArmToNearArmLen = 0.0f;
			float _defaultCenterLegToCeterArmLen = 0.0f;

			Vector3 _lastFrameCenterArmDirX = Vector3.zero;
			Vector3 _lastFrameCenterArmDirX2 = Vector3.zero;
			...
			void _SyncDisplacement()
			{
				if( _settings.syncDisplacement == SyncDisplacement.Everyframe || !_isSyncDisplacementAtLeastOnce ) {
					...
					if( _nearArmBones != null ) {
						...
						if( FBIKVecNormalize( ref dirY ) && FBIKComputeBasisFromXYLockY( out _centerLegToArmBasis, ref dirX, ref dirY ) ) {
                            _centerLegToArmBasisInv = _centerLegToArmBasis.transpose;
							FBIKMatMult( out _centerLegToArmBoneToBaseBasis, ref _centerLegToArmBasisInv, ref _internalValues.defaultRootBasis );
					...
					_defaultCenterLegToCeterArmLen = FBIKVecLength2( ref _defaultCenterLegPos, ref _defaultCenterArmPos );

					if( _footEffectors != null ) {
						if( _footEffectors[0].bone != null && _footEffectors[1].bone != null ) {
							_defaultCenterLegLen = FBIKVecLength2( ref _footEffectors[0].bone._defaultPosition, ref _footEffectors[1].bone._defaultPosition );
					...
					if( _armBones != null ) {
						...
						_solverCaches.armToArmLen = FBIKVecLength2( ref _armBones[0]._defaultPosition, ref _armBones[1]._defaultPosition );
					}

					if( _nearArmBones != null ) {
						_defaultNearArmToNearArmLen = FBIKVecLength2( ref _nearArmBones[0]._defaultPosition, ref _nearArmBones[1]._defaultPosition );
						if( _neckBone != null && _neckBone.transformIsAlive ) {
							_solverCaches.nearArmToNeckLength[0] = FBIKVecLength2( ref _neckBone._defaultPosition, ref _nearArmBones[0]._defaultPosition );
							_solverCaches.nearArmToNeckLength[1] = FBIKVecLength2( ref _neckBone._defaultPosition, ref _nearArmBones[1]._defaultPosition );
						}
					}
					if( _neckBone != null && _headBone != null ) {
						_solverCaches.neckToHeadLength = FBIKVecLength2( ref _neckBone._defaultPosition, ref _headBone._defaultPosition );
                    }
					...
					if( _spineBones != null ) {
						if( (_nearArmBones != null || _legBones != null) && _neckBone != null && _neckBone.transformIsAlive ) {
							...
							for( int i = 0; i < spineLength - 1; ++i ) {
								...
								if( !FBIKVecNormalize2( ref dirY0, ref dirY1 ) ) {
									continue;
								}

								if( !FBIKComputeBasisFromXYLockY( out prevToCenterArmBasis, ref armDirX, ref dirY0 ) ||
									!FBIKComputeBasisFromXYLockY( out currToNeckBasis, ref armDirX, ref dirY1 ) ) {
									continue;
								}

								FBIKMatMultInv0( out _spinePrevCenterArmToChildBasis[i], ref prevToCenterArmBasis, ref _spineBones[i]._localAxisBasis );
								FBIKMatMultInv0( out _spineCenterArmToChildBasis[i], ref currToNeckBasis, ref _spineBones[i]._localAxisBasis );
							}
						}
					}
				}
			...
			bool _UpperSolve()
			{
				...
				if( _settings.bodyIK.forceSolveEnabled ) {
					...
					if( hipsPull <= IKEpsilon && neckPull <= IKEpsilon && headPull <= IKEpsilon && armPull[0] <= IKEpsilon && armPull[1] <= IKEpsilon &&
				...
				if( continuousSolverEnabled ) {
					...
					if( _spineBones != null ) {
						for( int i = 0; i < _spineBones.Length; ++i ) {
							FBIKMatMultVecPreSubAdd( out temp.spinePos[i], ref centerLegBasis, ref _spineBones[i]._defaultPosition, ref _defaultCenterLegPos, ref baseCenterLegPos );
						}
					}
					if( _neckBone != null ) {
						FBIKMatMultVecPreSubAdd( out temp.neckPos, ref centerLegBasis, ref _neckBone._defaultPosition, ref _defaultCenterLegPos, ref baseCenterLegPos );
					}
					if( _headBone != null && temp.headEnabled ) {
						FBIKMatMultVecPreSubAdd( out temp.headPos, ref centerLegBasis, ref _headBone._defaultPosition, ref _defaultCenterLegPos, ref baseCenterLegPos );
					}
					for( int n = 0; n < 2; ++n ) {
						if( _shoulderBones != null ) {
							FBIKMatMultVecPreSubAdd( out temp.shoulderPos[n], ref centerLegBasis, ref _shoulderBones[n]._defaultPosition, ref _defaultCenterLegPos, ref baseCenterLegPos );
						}
						if( _armBones != null ) {
							FBIKMatMultVecPreSubAdd( out temp.armPos[n], ref centerLegBasis, ref _armBones[n]._defaultPosition, ref _defaultCenterLegPos, ref baseCenterLegPos );
						}
						if( _legBones != null ) {
							FBIKMatMultVecPreSubAdd( out temp.legPos[n], ref centerLegBasis, ref _legBones[n]._defaultPosition, ref _defaultCenterLegPos, ref baseCenterLegPos );
						}
					}
					temp.SetDirtyVariables();
				}
				...
				/*float dotProduct = Vector3.Dot(currentCenterArmDir, targetCenterArmDir);
				if (dotProduct < -0.3f)
				{ // Cross-body threshold for hand interactions
					upperLerpDir1Rate *= 0.2f; // Reduce hip rotation during cross-body movement
					upperLerpDir2Rate *= 0.2f;
				}*/
				...
				if (_lastFrameCenterArmDirX != Vector3.zero)
				{
					float dotProduct = Vector3.Dot(currentCenterArmDir, targetCenterArmDir);

					// Graduated smoothing based on how extreme the rotation is
					float smoothingFactor = 0.0f;
					float deadzoneThreshold = 0.0f;

					if (dotProduct < 0.3f)
					{
						// Extreme cross-body - heavy smoothing
						smoothingFactor = 0.95f;
						deadzoneThreshold = 3f;
					}
					else if (dotProduct < 0.7f)
					{
						// Moderate cross-body - medium smoothing  
					...
					else
					{
						...
						deadzoneThreshold = 1f;
					}

					float angleDiff1 = Vector3.Angle(centerArmDirX, _lastFrameCenterArmDirX);
					float angleDiff2 = Vector3.Angle(centerArmDirX2, _lastFrameCenterArmDirX2);

					if (angleDiff1 < deadzoneThreshold)
					{
						centerArmDirX = _lastFrameCenterArmDirX;
					}
					else
					{
						centerArmDirX = Vector3.Slerp(centerArmDirX, _lastFrameCenterArmDirX, smoothingFactor);
					}

					if (angleDiff2 < deadzoneThreshold)
					{
						centerArmDirX2 = _lastFrameCenterArmDirX2;
					}
					else
					{
						centerArmDirX2 = Vector3.Slerp(centerArmDirX2, _lastFrameCenterArmDirX2, smoothingFactor);
					}
				}
				_lastFrameCenterArmDirX = centerArmDirX;
				_lastFrameCenterArmDirX2 = centerArmDirX2;
				...
				if( !FBIKVecNormalize2( ref centerArmDirY, ref centerArmDirY2 ) ) {
					return false;
				}
					...
					if( _settings.bodyIK.spineAccurateLimitEnabled ) {
						...
						if( fromToYAng > spineLimitAngleY ) {
							...
							if( FBIKVecNormalize( ref axisDir ) ) {
								...
								Matrix3x3 rotateBasis;
								FBIKMatSetRot( out rotateBasis, ref q );
								FBIKMatMultVec( out centerArmDirY2, ref rotateBasis, ref centerArmDirY );
							}
						}
					} else {
						float fromToX = Vector3.Dot( centerArmDirX, centerArmDirX2 );
						...
						if( fromToXAng > spineLimitAngleX ) {
							if( fromToXAng > IKEpsilon ) {
								...
								if( FBIKVecNormalize( ref dirX2Balanced ) ) {
						...
						if( fromToYAng > spineLimitAngleY ) {
							if( fromToYAng > IKEpsilon ) {
								...
								if( FBIKVecNormalize( ref dirY2Balanced ) ) {
				...
				Vector3 presolveCenterLegPos = temp.centerLegPos;
				...
				if( _settings.bodyIK.upperSolveHipsEnabled ) {
					Matrix3x3 toBasis;
					if( FBIKComputeBasisFromXYLockY( out toBasis, ref centerArmDirX, ref centerArmDirY ) ) {
						...
						if( _internalValues.animatorEnabled || _internalValues.resetTransforms ) {
							if( continuousSolverEnabled && stableCenterLegRate > IKEpsilon ) {
								...
								if( FBIKVecNormalize( ref solveDirY ) && FBIKComputeBasisFromXYLockY( out presolveCenterLegBasis, ref solveDirX, ref solveDirY ) ) {
									Matrix3x3 tempBasis;
									FBIKMatFastLerp( out tempBasis, ref toBasis, ref presolveCenterLegBasis, stableCenterLegRate );
									toBasis = tempBasis;
								}
							}
							...
							if( FBIKVecNormalize( ref currentDirY ) && FBIKComputeBasisFromXYLockY( out fromBasis, ref currentDirX, ref currentDirY ) ) {
								FBIKMatMultInv1( out rotateBasis, ref toBasis, ref fromBasis );
							}
						} else {
							FBIKMatMultRet0( ref toBasis, ref _centerLegToArmBasisInv );
							...
							if( continuousSolverEnabled && stableCenterLegRate > IKEpsilon ) {
								...
								Vector3 solveDirX = centerArmDirX;
								if( FBIKVecNormalize( ref solveDirY ) && FBIKComputeBasisFromXYLockY( out presolveCenterLegBasis, ref solveDirX, ref solveDirY ) ) {
									FBIKMatMultRet0( ref presolveCenterLegBasis, ref _centerLegToArmBasisInv );
									Matrix3x3 tempBasis;
									FBIKMatFastLerp( out tempBasis, ref toBasis, ref presolveCenterLegBasis, stableCenterLegRate );
									toBasis = tempBasis;
								}
							...
							FBIKMatMultInv1( out rotateBasis, ref toBasis, ref centerLegBasis );
						}

						if( _settings.bodyIK.upperCenterLegLerpRate < 1.0f - IKEpsilon ) {
							FBIKMatFastLerpToIdentity( ref rotateBasis, 1.0f - _settings.bodyIK.upperCenterLegLerpRate );
						}
...

				{
					...
					for( int i = 0; i != spineLength; ++i ) {
						...
						if( i + 1 == spineLength ) {
							...
							if( !FBIKVecNormalize2( ref currentDirY, ref targetDirY ) ) {
								continue;
							}

							Matrix3x3 toBasis;
							FBIKComputeBasisFromXYLockY( out toBasis, ref targetDirX, ref targetDirY );
							Matrix3x3 fromBasis;
							FBIKComputeBasisFromXYLockY( out fromBasis, ref currentDirX, ref currentDirY );

							Matrix3x3 rotateBasis;
							FBIKMatMultInv1( out rotateBasis, ref toBasis, ref fromBasis );

							if( upperSpineLerpRate < 1.0f - IKEpsilon ) {
								FBIKMatFastLerpToIdentity( ref rotateBasis, 1.0f - upperSpineLerpRate );
							}
							...
							if( !FBIKVecNormalize4( ref currentDirX, ref currentDirY, ref targetDirY, ref targetDirY2 ) ) {
								continue;
							}
							...
							if( !FBIKComputeBasisFromXYLockY( out prevToNearArmBasis, ref currentDirX, ref targetDirY ) ||
								!FBIKComputeBasisFromXYLockY( out origToNearArmBasis, ref currentDirX, ref targetDirY2 ) ) {
								continue;
							}
							FBIKMatMultCol1( out targetDirY, ref prevToNearArmBasis, ref _spinePrevCenterArmToChildBasis[i] );
							FBIKMatMultCol1( out targetDirY2, ref origToNearArmBasis, ref _spineCenterArmToChildBasis[i] );
							...
							if( i + 1 != spineLength ) {
								...
								if( !FBIKVecNormalize( ref targetDirY ) ) {
			...
			void _UpperSolve_PresolveBaseCenterLegTransform( out Vector3 centerLegPos, out Matrix3x3 centerLegBasis )
			{
				...
				FBIKMatMultVecPreSubAdd( out legPos0, ref centerLegBasis, ref _legBones[0]._defaultPosition, ref _defaultCenterLegPos, ref centerLegPos );
				FBIKMatMultVecPreSubAdd( out legPos1, ref centerLegBasis, ref _legBones[1]._defaultPosition, ref _defaultCenterLegPos, ref centerLegPos );
			...
			void _UpperSolve_Transform( int origIndex, ref Matrix3x3 transformBasis )
			{
				...
				for( int i = 0; i != 2; ++i ) {
					FBIKMatMultVecPreSubAdd( out temp.armPos[i], ref transformBasis, ref temp.armPos[i], ref origPos, ref origPos );
					if( _shoulderBones != null ) {
						FBIKMatMultVecPreSubAdd( out temp.shoulderPos[i] , ref transformBasis , ref temp.shoulderPos[i] , ref origPos, ref origPos );
					}
				}

				int spineLength = (_spineBones != null) ? (_spineBones.Length) : 0;
				for( int spineIndex = origIndex + 1; spineIndex < spineLength; ++spineIndex ) {
					FBIKMatMultVecPreSubAdd( out temp.spinePos[spineIndex], ref transformBasis, ref temp.spinePos[spineIndex], ref origPos, ref origPos );
				}

				if( _neckBone != null ) {
					FBIKMatMultVecPreSubAdd( out temp.neckPos, ref transformBasis, ref temp.neckPos, ref origPos, ref origPos );
				}

				if( origIndex == -1 ) {
					if( temp.legPos != null ) {
						for( int i = 0; i < 2; ++i ) {
							FBIKMatMultVecPreSubAdd( out temp.legPos[i], ref transformBasis, ref temp.legPos[i], ref origPos, ref origPos );
						}
					}
				}
				...
				if( temp.PrepareLowerRotation( 0 ) ) {
					...
					for( int i = 0; i < 2; ++i ) {
						if( temp.legs.endPosEnabled[i] && temp.legs.targetBeginPosEnabled[i] ) {
							...
							if( FBIKVecNormalize( ref legDir ) ) {
			...
			void _ComputeWorldTransform()
			{
				...
				if( _hipsBone != null && _hipsBone.transformIsAlive && temp.spinePos != null && temp.spinePos.Length > 0 && _neckBone != null && _neckBone.transformIsAlive ) {
					...
					if( FBIKVecNormalize( ref dirY ) && FBIKComputeBasisFromXYLockY( out boneBasis, ref dirX, ref dirY ) ) {
						Matrix3x3 tempBasis;
						FBIKMatMult( out tempBasis, ref boneBasis, ref _centerLegBoneBasisInv );

						hipsToSpineDirX = boneBasis.column0;

						Quaternion worldRotation;
						FBIKMatMultGetRot( out worldRotation, ref tempBasis, ref _hipsBone._defaultBasis );
						...
						if( _hipsBone.isWritebackWorldPosition ) {
							...
							FBIKMatMultVecAdd( out worldPosition, ref tempBasis, ref inv_defaultLocalTranslate, ref temp.spinePos[0] );
							_hipsBone.worldPosition = worldPosition;
						}
					} else {
						if( FBIKVecNormalize( ref dirX ) ) {
                    ...
                    for( int i = 0; i != spineLength; ++i ) {
						...
						if( FBIKVecNormalize( ref dirY ) && FBIKComputeBasisFromXYLockY( out boneBasis, ref dirX, ref dirY ) ) {
							hipsToSpineDirX = boneBasis.column0;
							Quaternion worldRotation;
							FBIKMatMultGetRot( out worldRotation, ref boneBasis, ref _spineBones[i]._boneToWorldBasis );
					...
					if( _shoulderBones != null ) {
						for( int i = 0; i != 2; ++i ) {
							...
							if( FBIKVecNormalize3( ref xDir, ref yDir, ref zDir ) ) {
								boneBasis.SetColumn( ref xDir, ref yDir, ref zDir );
								Quaternion worldRotation;
								FBIKMatMultGetRot( out worldRotation, ref boneBasis, ref _shoulderBones[i]._boneToWorldBasis );
				...
				if( _solverInternal == null ) {
					...
					if( _spineUBone != null ) {
						if( _shoulderBones != null || _armBones != null ) {
							...
							if( FBIKVecNormalize3( ref dirX, ref dirY, ref dirZ ) ) {
			...
			void _PresolveHips()
			{
				...
				if( rotationEnabled ) {
					...
					FBIKMatGetRot( out centerLegRotationFrom, ref centerLegBasis );
				...
				if( positionEnabled ) {
					...
					FBIKMatMultVecPreSubAdd( out centerLegPos, ref centerLegBasis, ref _defaultCenterLegPos, ref _hipsEffector._defaultPosition, ref hipsEffectorWorldPosition );
			...
			void _ResetCenterLegTransform( ref Vector3 centerLegPos, ref Matrix3x3 centerLegBasis )
			{
				...
				if( _legBones != null ) {
					for( int i = 0; i != 2; ++i ) {
						FBIKMatMultVecPreSubAdd( out temp.legPos[i], ref centerLegBasis, ref _legBones[i]._defaultPosition, ref defaultCenterLegPos, ref centerLegPos );
                    }
				}
				if( _spineBones != null ) {
					for( int i = 0; i != _spineBones.Length; ++i ) {
						FBIKMatMultVecPreSubAdd( out temp.spinePos[i], ref centerLegBasis, ref _spineBones[i]._defaultPosition, ref defaultCenterLegPos, ref centerLegPos );
					}
				}
				if( _shoulderBones != null ) {
					for( int i = 0; i != 2; ++i ) {
						FBIKMatMultVecPreSubAdd( out temp.shoulderPos[i], ref centerLegBasis, ref _shoulderBones[i]._defaultPosition, ref defaultCenterLegPos, ref centerLegPos );
					}
				}
				if( _armBones != null ) {
					for( int i = 0; i != 2; ++i ) {
						FBIKMatMultVecPreSubAdd( out temp.armPos[i], ref centerLegBasis, ref _armBones[i]._defaultPosition, ref defaultCenterLegPos, ref centerLegPos );
					}
				}
				if( _neckBone != null ) {
					FBIKMatMultVecPreSubAdd( out temp.neckPos, ref centerLegBasis, ref _neckBone._defaultPosition, ref defaultCenterLegPos, ref centerLegPos );
				}
				if( _headBone != null && temp.headEnabled ) {
					FBIKMatMultVecPreSubAdd( out temp.headPos, ref centerLegBasis, ref _headBone._defaultPosition, ref defaultCenterLegPos, ref centerLegPos );
				}

				temp.SetDirtyVariables();
				temp._SetCenterLegPos( ref centerLegPos );
			}
			void _ResetShoulderTransform()
			{
				...
				if( FBIKVecNormalize( ref dirY ) && FBIKComputeBasisFromXYLockY( out boneBasis, ref dirX, ref dirY ) ) {
					Matrix3x3 tempBasis;
					FBIKMatMult( out tempBasis, ref boneBasis, ref _spineUBone._localAxisBasisInv );
					...
					for( int i = 0; i != 2; ++i ) {
						...
						if( _limbIK[limbIKIndex].IsSolverEnabled() ) {
							FBIKMatMultVecPreSubAdd( out temp.armPos[i], ref tempBasis, ref _armBones[i]._defaultPosition, ref _spineUBone._defaultPosition, ref tempPos );
                        }
                    }
				}
			}
			...
			class SolverInternal
			{
				public class Limb
				{
					...
					static bool _SolveTargetBeginPos( ref Vector3 targetBeginPos, ref Vector3 targetEndPos, ref FastLength targetBeginToEndLength, float endPull )
					{
						...
						if( beginToEndLengthSq > targetBeginToEndLength.lengthSq + FLOAT_EPSILON ) {
							float beginToEndLength = FBIKSqrt( beginToEndLengthSq );
				...
				Vector3 _lastFrameShoulderDir = Vector3.zero;
					...
					if( _isDirtyCenterLegBasis ) {
						...
						if( this.spinePos != null && this.spinePos.Length > 0 && legPos != null ) {
							...
							if( FBIKVecNormalize3( ref dirX, ref dirY, ref dirZ ) ) {
								...
								FBIKMatMultRet0( ref _centerLegBasis, ref _centerLegBoneBasisInv );
							}
						}
					}
				}
				...
				public void _UpdateSpineUBasis()
				{
					if( _isDirtySpineUBasis ) {
						...
						if( FBIKVecNormalize3( ref dirX, ref dirY, ref dirZ ) ) {
							...
							FBIKMatMultRet0( ref _spineUBasis, ref _spineUBoneLocalAxisBasisInv );
                        }
					}
				}
					...
					if( headMovingfixRate < 1.0f - IKEpsilon || bodyMovingfixRate < 1.0f - IKEpsilon ) {
						if( headPull > IKEpsilon || neckPull > IKEpsilon ) {
							if( headMovingfixRate < 1.0f - IKEpsilon && headPull > IKEpsilon ) {
								...
								if( _KeepMaxLength( ref tempNeckPos, ref _upperSolverTemp.targetHeadPos, _solverCaches.neckToHeadLength ) ) {
							...
							for( int i = 0; i != 2; ++i ) {
								if( bodyMovingfixRate < 1.0f - IKEpsilon && neckPull > IKEpsilon ) {
									...
									_upperSolverPreArmsTemp.nearArmPos[i] = Vector3.Lerp( _upperSolverPreArmsTemp.nearArmPos[i], tempNearArmPos, neckPull );
								} else {
									_KeepLength(
					...
					if( (enabled0 && enabled1) || neckHeadPull > IKEpsilon ) {
						for( int i = 0; i != 2; ++i ) {
							...
							if( _upperSolverArmsTemps[idx0].shoulderEnabled ) {
								...
								// Get spine-up direction for anatomical reference
								Vector3 spineUp = (neckPos - spineUPos).normalized;
								Vector3 currentShoulderDir = this.shoulderPos[1] - this.shoulderPos[0];
								Vector3 spineForward = Vector3.Cross(spineUp, currentShoulderDir).normalized;

								// Calculate target shoulder direction
								Vector3 calculatedDir = _upperSolverArmsTemps[idx0].shoulderPos[1] - _upperSolverArmsTemps[idx0].shoulderPos[0];

								// 1. SHOULDER WIDTH CONSTRAINT
								Vector3 shoulderCenter = (_upperSolverArmsTemps[idx0].shoulderPos[0] + _upperSolverArmsTemps[idx0].shoulderPos[1]) * 0.5f;
								float targetShoulderWidth = Vector3.Distance(_upperSolverArmsTemps[idx0].shoulderPos[0], _upperSolverArmsTemps[idx0].shoulderPos[1]);
								float naturalShoulderWidth = Vector3.Distance(this.shoulderPos[0], this.shoulderPos[1]);
								float maxShoulderWidth = naturalShoulderWidth * 1.15f;
								float minShoulderWidth = naturalShoulderWidth * 0.85f;

								if (targetShoulderWidth > maxShoulderWidth || targetShoulderWidth < minShoulderWidth)
								{
									float constrainedWidth = Mathf.Clamp(targetShoulderWidth, minShoulderWidth, maxShoulderWidth);
									Vector3 constrainedDir = calculatedDir.normalized;
									_upperSolverArmsTemps[idx0].shoulderPos[0] = shoulderCenter - constrainedDir * (constrainedWidth * 0.5f);
									_upperSolverArmsTemps[idx0].shoulderPos[1] = shoulderCenter + constrainedDir * (constrainedWidth * 0.5f);
									calculatedDir = _upperSolverArmsTemps[idx0].shoulderPos[1] - _upperSolverArmsTemps[idx0].shoulderPos[0];
									FBIKVecNormalize(ref calculatedDir);
								}

								// 4. CROSS-BODY DETECTION
								Vector3 leftArmToCenterBody = _upperSolverArmsTemps[idx0].armPos[0] - shoulderCenter;
								Vector3 rightArmToCenterBody = _upperSolverArmsTemps[idx0].armPos[1] - shoulderCenter;
								float leftCrossBodyFactor = Mathf.Clamp01(-Vector3.Dot(leftArmToCenterBody.normalized, currentShoulderDir.normalized));
								float rightCrossBodyFactor = Mathf.Clamp01(Vector3.Dot(rightArmToCenterBody.normalized, currentShoulderDir.normalized));
								float crossBodyIntensity = Mathf.Max(leftCrossBodyFactor, rightCrossBodyFactor);

								if (FBIKVecNormalize(ref currentShoulderDir) && FBIKVecNormalize(ref calculatedDir))
								{
									// Project both directions onto spine's frontal plane
									Vector3 projectedCalcDir = Vector3.ProjectOnPlane(calculatedDir, spineForward);
									Vector3 projectedCurrentDir = Vector3.ProjectOnPlane(currentShoulderDir, spineForward);

									if (FBIKVecNormalize(ref projectedCalcDir) && FBIKVecNormalize(ref projectedCurrentDir))
									{
										// 2. USE SYSTEM SHOULDER LIMITS
										float angleLimit = Mathf.Min(
											internalValues.bodyIK.shoulderLimitThetaYPlus._degrees,
											internalValues.bodyIK.shoulderLimitThetaZ._degrees
										) * 0.7f;

										// Scale constraints based on cross-body intensity
										if (crossBodyIntensity > 0.3f)
										{
											angleLimit *= Mathf.Lerp(1f, 0.5f, crossBodyIntensity);
										}

										float angleBetween = Vector3.Angle(projectedCurrentDir, projectedCalcDir);

										if (angleBetween > angleLimit)
										{
											Vector3 rotationAxis = Vector3.Cross(projectedCurrentDir, projectedCalcDir);
											if (FBIKVecNormalize(ref rotationAxis))
											{
												Quaternion maxRotation = Quaternion.AngleAxis(angleLimit, rotationAxis);
												_upperSolverArmsTemps[idx0].centerArmDir = maxRotation * projectedCurrentDir;
											}
											else
											{
												_upperSolverArmsTemps[idx0].centerArmDir = projectedCurrentDir;
											}
										}
										else
										{
											_upperSolverArmsTemps[idx0].centerArmDir = projectedCalcDir;
										}

										// 3. VELOCITY-BASED DAMPING
										if (_lastFrameShoulderDir != Vector3.zero)
										{
											float rotationSpeed = Vector3.Angle(_lastFrameShoulderDir, _upperSolverArmsTemps[idx0].centerArmDir);
											if (rotationSpeed > 8f)
											{
												float dampingFactor = Mathf.Clamp01(8f / rotationSpeed);
												_upperSolverArmsTemps[idx0].centerArmDir = Vector3.Slerp(_lastFrameShoulderDir, _upperSolverArmsTemps[idx0].centerArmDir, dampingFactor);
											}
										}
										_lastFrameShoulderDir = _upperSolverArmsTemps[idx0].centerArmDir;
									}
									else
									{
										_upperSolverArmsTemps[idx0].centerArmDir = currentShoulderDir;
									}
								}
								else
								{
									_upperSolverArmsTemps[idx0].centerArmDir = calculatedDir;
								}
						...
						if( !FBIKVecNormalize2( ref _upperSolverArmsTemps[0].centerArmDir, ref _upperSolverArmsTemps[1].centerArmDir ) ) {
							return false;
						}
						...
						if( !FBIKVecNormalize( ref _upperSolverArmsTemps[idx0].centerArmDir ) ) {
							return false;
						}
					...
					for( int i = 0; i != 2; ++i ) {
						...
						if( _limbIK[limbIKIndex].IsSolverEnabled() ) {
							...
							if( FBIKVecNormalize3( ref xDir, ref yDir, ref zDir ) ) {
								...
								FBIKMatMult( out _tempParentBasis[i], ref boneBasis, ref _shoulderBones[i]._boneToBaseBasis );
							}
							...
							if( _tempElbowPosEnabled[i] ) {
								FBIKMatMultCol0( out _tempArmToElbowDefaultDir[i], ref _tempParentBasis[i], ref _armBones[i]._baseToBoneBasis );
								if( i == 0 ) {
					...
					for( int i = 0; i != 2; ++i ) {
						if( _tempElbowPosEnabled[i] ) {
							...
							if( theta >= -FLOAT_EPSILON && theta <= FLOAT_EPSILON ) {
								...
								updateAnything = true;
								theta = FBIKCos( FBIKAcos( theta ) * feedbackRate );
								Matrix3x3 m = new Matrix3x3();
								FBIKMatSetAxisAngle( out m, ref axis, theta );

								Vector3 tempShoulderPos = this.shoulderPos[i];
								Vector3 tempDir = _tempArmPos[i] - tempShoulderPos;
								FBIKVecNormalize( ref tempDir );
								Vector3 resultDir;
								FBIKMatMultVec( out resultDir, ref m, ref tempDir );
				...
				bool _SolveLimbRotation( Limb limb, int origIndex, out Quaternion origRotation )
				{
					...
					origRotation = origRotation * origRotation;
					return true;
				}
				...
				public void UpperRotation( int origIndex, ref Matrix3x3 origBasis )
				{
...

					{
						...
						if( armPos != null ) {
							for( int i = 0; i < armPos.Length; ++i ) {
								FBIKMatMultVecPreSubAdd( out armPos[i], ref origBasis, ref armPos[i], ref origPos, ref origPos );
							}
						}
					}
					...
					if( shoulderPos != null ) {
						for( int i = 0; i < this.shoulderPos.Length; ++i ) {
							FBIKMatMultVecPreSubAdd( out shoulderPos[i], ref origBasis, ref shoulderPos[i], ref origPos, ref origPos );
						}
					}
					
					FBIKMatMultVecPreSubAdd( out neckPos, ref origBasis, ref neckPos, ref origPos, ref origPos );
					if( headEnabled ) {
						FBIKMatMultVecPreSubAdd( out headPos, ref origBasis, ref headPos, ref origPos, ref origPos );
					}
					if( origIndex == -1 ) {
						var legPos = this.legPos;
						if( legPos != null ) {
							for( int i = 0; i < legPos.Length; ++i ) {
								FBIKMatMultVecPreSubAdd( out legPos[i], ref origBasis, ref legPos[i], ref origPos, ref origPos );
                            }
						}
					...
					for( int t = (origIndex == -1) ? 0 : origIndex; t < this.spinePos.Length; ++t ) {
						FBIKMatMultVecPreSubAdd( out this.spinePos[t], ref origBasis, ref this.spinePos[t], ref origPos, ref origPos );
					}
				...
				public void LowerRotation( int origIndex, ref Matrix3x3 origBasis, bool bodyRotation )
				{
					...
					if( this.spinePos != null ) {
						...
						for( int n = 0; n < length; ++n ) {
							FBIKMatMultVecPreSubAdd( out spinePos[n], ref origBasis, ref spinePos[n], ref origPos, ref origPos );
						}
					}
					...
					if( !IsFuzzy( ref armPos[i], ref destArmPos ) ) {
						...
						if( FBIKVecNormalize( ref dirX ) ) {
							if( settings.bodyIK.shoulderLimitEnabled ) {
								...
								FBIKMatMultRet0( ref worldBasis, ref shoulderBones[i]._localAxisBasis );
								FBIKMatMultVecInv( out dirX, ref worldBasis, ref dirX );
								_LimitYZ_Square( i != 0, ref dirX, limitYMinus, limitYPlus, limitZ, limitZ );
								FBIKMatMultVec( out dirX, ref worldBasis, ref dirX );
							}
				...
				ref Vector3 rightLegPos )
			{
				...
				if( FBIKVecNormalize( ref dirY ) ) {
					return FBIKComputeBasisFromXYLockY( out centerLegBasis, ref dirX, ref dirY );
				} else {
			...
			static Quaternion _GetRotation( ref Vector3 axisDir, float theta, float rate )
			{
				if( (theta >= -IKEpsilon && theta <= IKEpsilon) || (rate >= -IKEpsilon && rate <= IKEpsilon) ) {
					...
					return Quaternion.AngleAxis( FBIKAcos( theta ) * rate * Mathf.Rad2Deg, axisDir );
				}
			}
