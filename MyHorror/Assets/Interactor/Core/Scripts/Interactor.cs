...
namespace razz
{
    ...
    {
        public const float Version = 0.999f;
        ...
        public bool playerUsableMultiple { get; set; }
        public GameObject usableMultipleObject { get; set; }
        ...
        public static Interactor Instance
        {
            get
            {
                if (_instance == null)
				    ...
				    Instance = FindFirstObjectByType<Interactor>();
#else
        ...
        private void Awake()
        {
            ...
            if (InteractorIK.defaultFiles != 0 && fbikEnabled) fbikEnabled = false;
            ...
            else interactorIK.interactor = this;
        ...
        private void DistanceObjRay()
        {
            ...
            _mousePosRay = _mainCam.ScreenPointToRay(InteractorInput.GetMousePosition());
        ...
        //Since we only use distance to sort objects and for range based interactions, instead of using expensive distance call, we're using sqrMag.
        ...
        private void IntObjEnter(GameObject Object)
        {
            ...
            if (!intObj.enabled) return;

            if (intObj.interactionType != InteractionTypes.DistanceCrosshair && intObj.interactionType != InteractionTypes.SelfPathMover)
            {
            ...
            if (intObj.interactionType == InteractionTypes.SelfPathMover)
            {
                temp.distanceSqr = -1;
            }
            else
            ...
            if (selectedByUI < intObjComponents.Count && intObj.interactionType != InteractionTypes.SelfPathMover && intObjComponents[selectedByUI].interactorObject == intObj)
            {
        ...
        private bool IsInteractableForAllTargets(InteractorObject interactorObject)
        {
            ...
            for (int i = 0; i < effectorLinks.Count; i++)
            {
                ...
                if (!effectorLinks[i].EffectorCheckOrbitalPosition(target.transform, interactorObject))
                {
                    useable = false;
                    break;
                }
        ...
        {
            if ((int)GetEffectorInteractingWith(interactorObject) < 0) return false;
            else return true;
        }
        ...
        public bool AreAnyEffectorsInteracting()
        {
            for (int i = 0; i < effectorLinks.Count; i++)
            {
                if (!effectorLinks[i].enabled) continue;

                if (effectors[i].connected) return true;
            }
            return false;
        }
        ...
        public void Connect(int arrayPlace, InteractorObject connectedTo, InteractorTarget connectedTarget)
        {
            ...
            if (connectedTo.currentInteractor != this)
            {
                connectedTo.AssignInteractor(this);
            }
        }
        ...
        {
            if (effectors[arrayPlace].connectedTo && effectors[arrayPlace].connectedTo.orbitalReach && orbitalReach && !_orbitalReach.DidResumed(effectors[arrayPlace].connectedTo))
            {
                _orbitalReach.ResumeOrbitalAnim(effectors[arrayPlace].connectedTo);
            }
        ...
        {
            if (!interactorObject) return false;
        ...
        public void AddInteractionManual(InteractorObject interactorObject)
        {
            ...
            if (selectedByUI < intObjComponents.Count && interactorObject.interactionType != InteractionTypes.SelfPathMover && intObjComponents[selectedByUI].interactorObject == interactorObject)
            {
        ...
        public bool ForcePick(InteractorObject interactorObject, bool orbitalIncluded)
        {
            for (int i = 0; i < interactorObject.childTargets.Length; i++)
            {
                ...
                if (effectors[effectorIndex].connected && effectors[effectorIndex].connectedTo)
                {
                    if (_interactorIK.GetProgress(effectorLinks[effectorIndex].effectorType) > 1.98f || (_interactorIK.GetProgress(effectorLinks[effectorIndex].effectorType) == 0f))
                    {
                        Disconnect(effectorIndex);
                    }
                    else continue;
                }
        ...
        private void AddNewDebugData(InteractorObject intObj)
        {
            ...
            if (intObj.interactionType == InteractionTypes.DistanceCrosshair || intObj.interactionType == InteractionTypes.DistanceRange || intObj.interactionType == InteractionTypes.DefaultOnce || intObj.interactionType == InteractionTypes.SelfPathMover) return;
        ...
        public class EffectorLink
        {
            ...
            private bool EffectorCheck(InteractorTarget target, Transform targetTransform)
            {
                ...
                if (target && target.overrideEffector)
                {
            ...
            public bool EffectorCheckOrbitalPosition(Transform target, InteractorObject interactorObject = null)
            {
                InteractorObject checkingIntObj;
                if (interactorObject == null)
                    checkingIntObj = _interactor.effectors[_this].connectedTo;
                else checkingIntObj = interactorObject;

                if (_interactor.orbitalReach && checkingIntObj && checkingIntObj.orbitalReach) return OrbitalCheck(target);
                else
                {
                    if (!_interactor.orbitalReach && checkingIntObj && checkingIntObj.orbitalReach)
                    {
            ...
            public void ObjectExit(InteractorObject exitInteractorObject)
            {
                ...
                if (exitInteractorObject.interactionType == InteractionTypes.DistanceCrosshair || exitInteractorObject.interactionType == InteractionTypes.MultipleMoveBody || exitInteractorObject.interactionType == InteractionTypes.SelfPathMover || exitInteractorObject.interactionType == InteractionTypes.ManualButton || exitInteractorObject.interactionType == InteractionTypes.DefaultOnce || exitInteractorObject.interactionType == InteractionTypes.DistanceRange)
                {
                    _interactor.playerUsable = false;
                    exitInteractorObject.RemoveEffectorFromUseables(-1);
                    exitInteractorObject.used = false;
                }

                if (exitInteractorObject.orbitalReach && _interactor.orbitalReach && exitInteractorObject.itemForInventory && exitInteractorObject.itemForInventory.currentInventory)
                {
                    return;
                }
                ...
                {
                    if (exitInteractorObject.interactionType == InteractionTypes.PickableOne)
                    {
                        _interactor.Disconnect(_this);
                        return;
                        // Let the hand finish returning, main loop will handle disconnect
                    }
                    StopInteraction(effectorType);
                }
            }
            ...
            public void StartInteraction(FullBodyBipedEffector effector, InteractorTarget interactorTarget, InteractorObject interactorObject)
            {
                ...
                if (interactorObject.interactionType != InteractionTypes.SelfPathMover)
            ...
            public bool PrepareOrbitalInteraction(InteractorObject intObj, InteractorTarget currentTarget)
            {
                ...
                if (!mirror && (effectorType != FullBodyBipedEffector.RightHand && effectorType != FullBodyBipedEffector.Body))
                {
                    Debug.LogWarning("OrbitalReach is only for Left/Right Hand or Body targets.", currentTarget);
            ...
            public void Update(List<IntObjComponents> interactionObjsInRange, Vector3 sphereColWithRotScale)
            {
                ...
                if (_interactor.effectors[_this].connected)
                {
                    ...
                    {
                        if (_interactor.effectors[_this].connectedTo && _interactor.effectors[_this].connectedTo.itemForInventory)
                        {
                            if (!_interactor.effectors[_this].connectedTo.itemForInventory.currentInventory)
                            {
                                if (_interactor.effectors[_this].connectedTo)
                ...
                for (int objPlaceInList = 0; objPlaceInList < interactionObjsInRange.Count; objPlaceInList++)
                {
                    ...
                    {
                        if (!_interactorObject.itemForInventory) interactionObjsInRange.Remove(interactionObjsInRange[objPlaceInList]);
                        else if(!_interactorObject.itemForInventory.currentInventory) interactionObjsInRange.Remove(interactionObjsInRange[objPlaceInList]);
                        continue;
                    }
                    ...
                    {
                        case InteractionTypes.DefaultOnce:
                            {//Default (int)10-20
                                for (int i = 0; i < _allTargets.Length; i++)
                                {
                                    ...
                                    {
                                        bool passed = EffectorCheck(_allTargets[i]);
                                    ...
                                    else
                                    {
                                        if (!EffectorCheck(_allTargets[i]))
                                        {
                                            ...
                                            _interactorObject.RemoveEffectorFromUseables(-1);
                            ...
                            {//Manual (int)20-30
                                for (int i = 0; i < _allTargets.Length; i++)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (_interactorIK.GetProgress(effectorType) == 2f || (_interactorIK.GetProgress(effectorType) == 0 && _eventSent))
                                        {
                                            _interactor.playerUsable = true;
                                            _interactorObject.RemoveEffectorFromUseables((int)effectorType);
                                            _interactorObject.used = false;
                                            _eventSent = false;
                                            _interactor.NewLookOrder(_interactorObject, Look.Never);
                        ...
                        case InteractionTypes.DistanceRange:
                            {
                                ...
                                if (interactionObjsInRange[objPlaceInList].distanceSqr <= _interactorObject.distanceSettings.range * _interactorObject.distanceSettings.range)
                                {
                        ...
                        case InteractionTypes.MultipleMoveBody:
                            {//Multiple (int)60-70
                                for (int i = 0; i < _allTargets.Length; i++)
                                {
                                    if ((int)_allTargets[i].effectorType != (int)effectorType) continue;
                                    ...
                                    GatherDebugDataStart(_allTargets[i], _interactorObject, InteractionTypes.MultipleMoveBody);
                                    ...
                                    if (passed)
                                    {
                                        ...
                                        _interactorObject.AddEffectorToUseables((int)effectorType);
                                        _selfPossible = false;
                                        _interactor.usableMultipleObject = _interactorObject.transform.root.gameObject;
                                        _interactor.playerUsableMultiple = true;
                                    }
                                    else
                                    {
                        ...
                        case InteractionTypes.SelfPathMover:
                            {//Self (int)70-80
                                ...
                                for (int i = 0; i < _allTargets.Length; i++)
                                {
                                    ...
                                    if (!_interactor.effectors[_this].connected && _selfPossible && !_interactorObject.selfSettings.selfActive)
                                    {
                                        ...
                                        {
                                            if (_interactor.playerIdle && !_interactorIK.IsInInteraction(_allTargets[i].effectorType))
                                            {
                                                _interactorObject.AddEffectorToUseables((int)effectorType);
                                                _eventSent = false;
                                    ...
                                    {
                                        if (!_interactor.playerIdle || !_interactorObject.selfSettings.pathMovers[i].playing)
                                        {
                                ...
                                for (int i = 0; i < _allTargets.Length; i++)
                                {
                                    ...
                                    else
                                    {
                                        if (_interactorIK.IsPaused(effectorType))
                                        {
                                            ...
                                            {
                                                if (!_interactorObject.pickableOneSettings.Pick(_interactor, (int)effectorType))
                                                {
                                                    _interactorIK.ResumeInteraction(effectorType);
                                                    _interactor.Disconnect(_this);
                                                    ResetInteractorObject(_interactorObject);
                                    ...
                                    if (_interactor.effectors[_this].connected)
                                    {
                                        if (_interactorIK.GetProgress(effectorType) > 1.98f || (_interactorIK.GetProgress(effectorType) == 0f && _eventSent))
                                        {
                                            _eventSent = false;
                                            _interactor.Disconnect(_this);
                                        }
                        ...
                        case InteractionTypes.Push:
                            {//Push & Pull (int)90-100
                                for (int i = 0; i < _allTargets.Length; i++)
                                {
                                    ...
                                    {
                                        if (_interactorIK.IsPaused(_interactor.effectors[_this].connectedTarget.effectorType) && !_interactor.playerPushing)
                                        {
                                            _interactor.effectors[_this].connectedTo.pushSettings.PushStart(_playerTransform);
                                            _interactor.playerPushing = true;
                                            _interactorObject.SendUnityEvent();
                                            _interactor.NewLookOrder(_interactorObject, Look.After);
                                        }
                                    }
                ...
                if (selectedByUI < intOjbComponents.Count)
                {
                    ...
                    if (_interactor.effectors[_this].connected)
                    {
                        ...
                        if (startStopObject != _interactor.effectors[_this].connectedTo && !SkipInterruption(currentType))
                        {
                            ...
                            {
                                if (_interactor.effectors[_this].connectedTo.interactionType != InteractionTypes.MultipleMoveBody)
                                {
                        ...
                        switch (_interactor.effectors[_this].connectedTo.interactionType)
                        {
                            ...
                            case InteractionTypes.Push:
                                {
                                    _interactor.playerPushing = false;
                                ...
                                case InteractionTypes.MultipleMoveBody:
                                    {
                                        ...
                                        if (!_interactor.playerUsableMultiple) continue;
