...
namespace razz
{
    ...
    public class InteractorEditor : Editor
    {
        ...
        private void TopRightLinks()
        {
            ...
            GUILayout.Space(2f);
#endif
            if (GUILayout.Button(new GUIContent("Welcome Window", "Import Examples or read what's new about Interactor."), _smallButton))
            {
        ...
        public override void OnInspectorGUI()
        {
            ...
            else
            {
                ...
                if (!_defaultFiles && _script.fbikEnabled)
                {
                    _script.fbikEnabled = false;
                }

                if (_script.fbikEnabled)
                {
                    Color temp = GUI.contentColor;
                    GUI.contentColor = Color.green;
                    if (WindowGUI.ButtonValue("Interactor Full Body IK is ON ", "Disable Interactor Full Body IK for this character. When disabled, only Unity IK will be used, which controls only the arms and legs. Full Body IK, on the other hand, controls the entire body, allowing it to pull the body towards the target when they are out of reach."))
                    {
                        _script.fbikEnabled = false;
                    }
                ...
                else
                {
                    ...
                    if (WindowGUI.ButtonValue("Interactor Full Body IK is OFF", "Enable Interactor Full Body IK for this character. When disabled, only Unity IK will be used, which controls only the arms and legs. Full Body IK, on the other hand, controls the entire body, allowing it to pull the body towards the target when they are out of reach."))
                    {
                        if (!_defaultFiles)
                        {
                            Debug.LogWarning("Switch to Default Interactor IK first, Interactor Full Body IK can not be used with Final IK integration!");
                            GUI.contentColor = temp;
                            return;
                        }
                        else _script.fbikEnabled = true;
                        GUI.contentColor = temp;
                    }
                }
                ...
                if (animator = _script.GetComponentInChildren<Animator>())
                {
                    ...
                    if ((int)type == 0)
                    {
                        ...
                        if (target)
                        {
                            ...
                            createdTarget.localScale = target.lossyScale;
            ...
            Color temp2 = GUI.contentColor;
            if (_script.fbikEnabled)
            {
                GUI.contentColor = Color.green;
                if (WindowGUI.ButtonValue("Full Body IK is ON ", "Disable Interactor Full Body IK for this character. When disabled, only Unity IK will be used, which controls only the arms and legs. Full Body IK, on the other hand, controls the entire body, allowing it to pull the body towards the target when they are out of reach."))
                {
                    _script.fbikEnabled = false;
                }
            ...
            {
                GUI.contentColor = Color.red;
                if (WindowGUI.ButtonValue("Full Body IK is OFF", "Enable Interactor Full Body IK for this character. When disabled, only Unity IK will be used, which controls only the arms and legs. Full Body IK, on the other hand, controls the entire body, allowing it to pull the body towards the target when they are out of reach."))
                {
                    if (!_defaultFiles)
                    {
                        Debug.LogWarning("Switch to Default Interactor IK first, Interactor Full Body IK can not be used with Final IK integration!");
        ...
        private void AdjustCollider(Animator animator)
        {
            ...
            if (head && foot)
            {
                ...
                Vector3 upDir = _script.transform.up;
                _script.transform.rotation = Quaternion.LookRotation(Vector3.forward, Vector3.up);
                float dist = Mathf.Abs(head.position.y - foot.position.y);
        ...
        private void AdjustInteractorIK(Animator animator, InteractorIK.IKPart part)
        {
            ...
            {
                InteractorIK interactorIK = _script.GetComponentInChildren<InteractorIK>();
            ...
            if (InteractorIK.defaultFiles != 0 && _script.fbikEnabled)
            {
                _script.fbikEnabled = false;
            }
            ...
            if (_script.fbikEnabled)
            {
                _script.interactorIK.SetInteractorFBIK();
            }
        ...
        {
            HashSet<string> exampleScenes = new HashSet<string>
            {
                "01_ProtoMan",
                "02_UnityArmature",
                "03_InteractorPaths",
                "04_InteractorAi",
                "05_Pickables and Inventories",
                "06_Pickables Hit"
            };
            ...
            if (!exampleScenes.Contains(sceneName)) return;
            if (AssetDatabase.LoadAssetAtPath<SceneAsset>(currentScene.path) == null) return;

            UnityEditor.SceneManagement.EditorSceneManager.OpenScene(currentScene.path);
            Debug.Log(sceneName + " scene was reopened...");
        }
        #endregion
