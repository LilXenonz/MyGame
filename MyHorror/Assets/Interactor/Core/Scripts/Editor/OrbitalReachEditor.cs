    ...
    public class OrbitalReachEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!Application.isPlaying)
            {
                ...
                if (GUILayout.Button(new GUIContent("Add OrbitalReachLayer", "Adds OrbitalReachLayer to Animator Controller")))
                    AddOrbitalReachLayer();
                if (GUILayout.Button(new GUIContent("Remove OrbitalReachLayer", "Removes OrbitalReachLayer from Animator Controller")))
                    RemoveOrbitalReachLayer(true);
                GUILayout.EndHorizontal();
            }

            DrawTimingSystemInfo();
        ...
        private void DrawTimingSystemInfo()
        {
            if (!Application.isPlaying) return;

            GUILayout.Space(10f);
            EditorGUILayout.LabelField("Timing System Info", EditorStyles.boldLabel);

            if (_script.animator)
            {
                string updateMode = "Unknown";
#if UNITY_2023_1_OR_NEWER
                updateMode = _script.animator.updateMode.ToString();
#else
                updateMode = _script.animator.updateMode == AnimatorUpdateMode.AnimatePhysics ? "AnimatePhysics" : "Normal";
#endif
                EditorGUILayout.LabelField("Animator Update Mode", updateMode);
            }

            EditorGUILayout.LabelField("Use Unscaled Time", _script.useUnscaledTime.ToString());
            EditorGUILayout.LabelField("Sync With Fixed Update", _script.syncWithFixedUpdate.ToString());
            EditorGUILayout.LabelField("Frame Rate Independent", _script.frameRateIndependent.ToString());

            if (_script.useUnscaledTime)
                EditorGUILayout.LabelField("Current Time Source", "Time.unscaledTime");
            else if (_script.syncWithFixedUpdate)
                EditorGUILayout.LabelField("Current Time Source", "Time.fixedTime");
            else
                EditorGUILayout.LabelField("Current Time Source", "Time.time");
        }
        ...
        private bool CheckOrbitalAnimClips()
        {
            ...
            for (int i = 0; i < _orbitalAnimClips.Length; i++)
            {
                ...
                if (clip == null)
                {
                    Debug.LogWarning($"OrbitalReach animation clip '{_orbitalAnimClips[i]}' could not be found at path: {_folderPath + _orbitalAnimClips[i] + _file}");
                    return false;
                }
            }
            return true;
        ...
        private void RemoveOrbitalReachLayer(bool button)
        {
            ...
            {
                if (button) Debug.Log("OrbitalReachLayer could not be found.");
                return;
            }

            _animatorController.RemoveLayer(_orbitalLayerIndex);
            if (button) Debug.Log("OrbitalReachLayer is removed from Animator Controller.");
            else if (button) Debug.Log("OrbitalReachLayer could not be found.");
        ...
        private void UpdateDebug()
        {
            ...
            _script.animator.SetLayerWeight(_script.orbitalLayerIndex, _script.debugOrbitalValues.z);
            ...
            if (_script.debugOrbitalValues.x < _script.crouchWeightNormalized)
                ...
                _script.animator.CrossFade(OrbitalReach.StandState, 0f, _script.orbitalLayerIndex, _script.debugDuration);
