...
namespace razz
{
    internal sealed class SceneviewPopup : PopupWindowContent
    {
        ...
        private Dictionary<GameObject, float> _nameWidths = new Dictionary<GameObject, float>();
        ...
        private void PrecalculateRequiredSizes()
        {
            ...
            for (int i = 0; i < _prefabs.Count; i++)
            {
                ...
                var content = new GUIContent(_prefabs[i].name);
                float fullWidth = _styles.label.CalcSize(content).x;
                _nameWidths[_prefabs[i]] = fullWidth;

                if (fullWidth > _buttonWidth)
                {
                    _buttonWidth = fullWidth + 80f;
                }
            }

            _buttonWidth = Mathf.Max(_buttonWidth, 200f);
            _buttonWidth += EditorGUIUtility.standardVerticalSpacing;
            ...
            for (int i = 0; i < _prefabs.Count; i++)
            {
                ...
                _iconLookup[_prefabs[i]] = _displayedIcons.ToArray();
        ...
        public override void OnGUI(Rect rect)
        {
            ...
            float contentHeight = RowHeight() * _prefabs.Count;
            bool needsScroll = contentHeight > rect.height;

            Rect windowRect = new Rect(0, 0, EditorGUIUtility.currentViewWidth,
                _prefabs.Count == 1 ? RowHeight() : rect.height);
            GUI.Box(windowRect, "", GUI.skin.GetStyle("BackgroundStyle"));

            if (needsScroll)
            {
                _scroll = GUI.BeginScrollView(rect, _scroll, _contentRect,
                    _skin.GetStyle("horizontalscrollbar"),
                    _skin.GetStyle("verticalscrollbar"));
            }

            Rect itemRect = rect;
            itemRect.height = EditorGUIUtility.singleLineHeight + 2;
            itemRect.y = _prefabs.Count == 1 ? 0 : EditorGUIUtility.standardVerticalSpacing - 1;
            itemRect.xMin += 2;
            itemRect.xMax -= (needsScroll ? 16 : 2);
            itemRect.width = _contentRect.width - (needsScroll ? 16 : 2);

            using (new EditorGUIUtility.IconSizeScope(_styles.iconSize))
            {
                for (int i = 0; i < _prefabs.Count; i++)
                {
                    DrawRow(itemRect, current, _prefabs[i]);
                    itemRect.y += RowHeight();

                    if (i < _prefabs.Count - 1)
                        DrawSplitter(itemRect);
                }
            }
        ...
        private void DrawRow(Rect rect, Event current, GameObject target)
        {
            ...
            float iconsWidth = icons.Length * 18;

            rect.x = iconRect.xMax;
            rect.width = rect.width - iconRect.width - iconsWidth - 4;

            var nameContent = _styles.TempContent(target.name, null);
            EditorGUI.LabelField(rect, nameContent, GUI.skin.GetStyle("grey_border"));

            if (current.type == EventType.MouseDown && originalRect.Contains(current.mousePosition))
            {
            ...
            Rect componentIconRect = originalRect;
            componentIconRect.x = originalRect.xMax - iconsWidth;
            componentIconRect.width = 18;

            for (int i = 0; i < icons.Length; i++)
            {
                EditorGUI.LabelField(componentIconRect, _styles.TempContent(null, icons[i]));
                componentIconRect.x += componentIconRect.width;
            }
        }

        public override Vector2 GetWindowSize()
        {
            float maxNameWidth = _nameWidths.Values.Max();
            float width = 22f + maxNameWidth + _iconWidth + 60f;
            width = Mathf.Max(width, 300f);
            ...
            {
                height = (RowHeight() * _prefabs.Count) + EditorGUIUtility.standardVerticalSpacing;
            }

            float minHeight = _prefabs.Count == 1 ? height : Mathf.Max(EditorGUIUtility.singleLineHeight * 2, height);
            float maxHeight = Mathf.Min(Screen.currentResolution.height * 0.8f, 800);

            bool needsScroll = height > maxHeight;

            var size = new Vector2(
                width + (needsScroll ? 14 : 0),
                Mathf.Clamp(height, minHeight, maxHeight)
            );

            _contentRect = new Rect(Vector2.zero, new Vector2(width, height));
