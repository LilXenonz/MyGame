...
namespace razz
{
    ...
    public class InteractorTargetEditor : Editor
    {
        ...
        private SerializedProperty p_elbowPoleHorizontal;
        private SerializedProperty p_elbowPoleVertical;
        ...
        private SerializedProperty p_pinTargetWhenDone;
        ...
        private void SetProperties()
        {
            ...
            p_elbowPoleHorizontal = _so.FindProperty("elbowPoleHorizontal");
            p_elbowPoleVertical = _so.FindProperty("elbowPoleVertical");
            ...
            p_pinTargetWhenDone = _so.FindProperty("pinTargetWhenDone");
        ...
        private bool GetEffectorValues()
        {
            ...
            Interactor _interactor = FindFirstObjectByType<Interactor>();
#else
        ...
        private void ShowBonesBasic(SceneView sceneView)
        {
            ...
            if (_rootNode != null && Selection.activeTransform != null)
            {
                ...
                float scaledBoneSize = _boneGizmosSize * Mathf.Max(_script.transform.lossyScale.x, _script.transform.lossyScale.y, _script.transform.lossyScale.z);
                if (Handles.Button(_script.transform.position, Quaternion.LookRotation(_script.transform.up, -_script.transform.forward), scaledBoneSize * 2f, scaledBoneSize * 2f, Handles.CircleHandleCap))
                {
        ...
        private void ShowBones(SceneView sceneView)
        {
            ...
            if (_rootNode != null && Selection.activeTransform != null)
            {
                Handles.color = _targetColor;
                float scaledBoneSize = _boneGizmosSize * Mathf.Max(_script.transform.lossyScale.x, _script.transform.lossyScale.y, _script.transform.lossyScale.z);
                if (Handles.Button(_script.transform.position, Quaternion.LookRotation(_script.transform.up, -_script.transform.forward), scaledBoneSize * 3f, scaledBoneSize * 2f, Handles.CircleHandleCap))
                {
                    Selection.activeGameObject = _script.gameObject;
                }
                ...
                foreach (var bone in _children)
                {
                    ...
                    if (!_speedPreview)
                    {
                        if (Handles.Button(bone.transform.position, rot, scaledBoneSize, scaledBoneSize, Handles.CircleHandleCap))
                    ...
                    Handles.DrawWireCube(Vector3.zero, new Vector3(scaledBoneSize, scaledBoneSize, (end - start).magnitude));
                    Handles.matrix = matr;
        ...
        public override void OnInspectorGUI()
        {
            ...
            else if (p_effectorType.enumValueIndex == 5 || p_effectorType.enumValueIndex == 6)
            {
                GUILayout.BeginHorizontal();
                EditorGUILayout.PropertyField(p_elbowPoleHorizontal, new GUIContent("Elbow Pole Horizontal", "Sets elbow bending direction to side or back when using this target. (Not active on Final IK version)"));
                p_elbowPoleHorizontal.floatValue = Mathf.Clamp01(p_elbowPoleHorizontal.floatValue);

                EditorGUILayout.PropertyField(p_elbowPoleVertical, new GUIContent("Elbow Pole Vertical", "Sets elbow bending direction to up or down when using this target. (Not active on Final IK version)"));
                p_elbowPoleVertical.floatValue = Mathf.Clamp01(p_elbowPoleVertical.floatValue);
                GUILayout.EndHorizontal();
            }
            GUILayout.BeginHorizontal();
            ...
            {
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.Slider(p_pinTargetWhenDone, 0f, 1f, new GUIContent("Pin Target When Done", "When interaction is done and body part starts to return to its default animation position and rotation on back path (second half), if target is on the move, last OnPause position and rotation will be stored(relative to player) upon leaving the object. The path will then be interpolated between these recorded values and the actual target values of the object (0f for actual target, 1f for stored last values before leaving the object). Useful for targets that move away after the interaction, such as throwing an object. Only enabled on Straight Mode, does not work with Bezier mode."));
                EditorGUILayout.EndHorizontal();
            }
            else _script.pinTargetWhenDone = 0f;
