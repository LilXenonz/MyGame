...
namespace razz
{
    ...
    public class OrbitalReach : MonoBehaviour
    {
        ...
        [Tooltip("Forces consistent timing regardless of time scale, frame rate, or animator settings. Recommended for most cases.")]
        public bool useUnscaledTime = true;
        [Tooltip("Sync timing with FixedUpdate for physics-based characters. Use when animator is on FixedUpdate.")]
        public bool syncWithFixedUpdate = false;
        [Tooltip("Frame rate independent timing. Helps with consistency across different devices.")]
        ...
        private bool _isFixedUpdateMode;
        ...
        private void InitializeTimingSystem()
        {
            ...
            _isFixedUpdateMode = animator.updateMode == AnimatorUpdateMode.Fixed;
#else
            _isFixedUpdateMode = animator.updateMode == AnimatorUpdateMode.AnimatePhysics;
#endif

            if (syncWithFixedUpdate && _isFixedUpdateMode)
            {
        ...
        private float GetCurrentTime()
        {
            ...
            else if (syncWithFixedUpdate && _isFixedUpdateMode)
        ...
        private void UpdateTimingSystem()
        {
            ...
            _timeDelta = _currentTime - _lastUpdateTime;
            _lastUpdateTime = _currentTime;

            if (frameRateIndependent)
            {
                _timeDelta = Mathf.Clamp(_timeDelta, 0f, 0.1f);
            }
        }

        private object GetWaitInstruction(float duration)
        {
            if (useUnscaledTime)
        ...
        public bool SetOrbitalReach(InteractorObject interactorObject, InteractorTarget interactorTarget, Vector3 orbitalValues, bool mirror, int arrayPlace, bool returning)
        {
            ...
            StartCoroutine(UniversalAnimCoroutine(interactorObject, interactorTarget, arrayPlace, lateTargetDur, lateBackDur, returning));

            return true;
        }

        private IEnumerator UniversalAnimCoroutine(InteractorObject intObj, InteractorTarget interactorTarget, int arrayPlace, float lateTarget, float lateBack, bool returning)
        {
            clipPlaying = true;
            ...
            yield return StartCoroutine(BlendLayerWeight(0, _lastWeight, lateTarget));

            interactor.StartStopInteractionOrbital(intObj, interactorTarget, arrayPlace);
            if (returning) intObj.pickableOneSettings.Drop();

            yield return StartCoroutine(WaitForDuration(intObjTargetDur));

            if (returning)
            {
                animator.SetFloat(OrbitalSpeed, -_lastOrbitalBackSpeed);
                yield return StartCoroutine(BlendLayerWeight(_lastWeight, 0, lateBack));
                clipPlaying = false;
                yield break;
            }
            ...
            if (intObj.pauseOnInteraction)
            {
                ...
                if (intObj.pauseTime > 0)
                {
                    float pauseStartTime = GetCurrentTime();
                    yield return new WaitUntil(() => DidResumed(intObj) || (GetCurrentTime() - pauseStartTime) >= intObj.pauseTime);

                    if (!_resumed && (GetCurrentTime() - pauseStartTime) >= intObj.pauseTime)
                    {
                        _resumed = true;
                    }
                }
            ...
            animator.SetFloat(OrbitalSpeed, _lastOrbitalBackSpeed);
            yield return StartCoroutine(WaitForDuration(intObjBackDur));

            yield return StartCoroutine(BlendLayerWeight(_lastWeight, 0, lateBack));
            clipPlaying = false;
        }

        private IEnumerator BlendLayerWeight(float fromWeight, float toWeight, float duration)
        {
            if (duration <= 0f)
            {
                animator.SetLayerWeight(orbitalLayerIndex, toWeight);
                yield break;
            }
            ...
            while (elapsedTime < duration)
            {
                ...
                float lerpedWeight = Mathf.Lerp(fromWeight, toWeight, t);
        ...
        private IEnumerator WaitForDuration(float duration)
        {
            ...
            {
                yield return GetWaitInstruction(duration);
            }
        }
        ...
        public void ResumeOrbitalAnim(InteractorObject intObj)
        {
        ...
        public void ReverseOrbitalAnim(InteractorObject intObj)
        {
            if (intObj == _currentIntObj)
            {
                ...
                _resumed = true;
                clipPlaying = false;
                StopCoroutine("UniversalAnimCoroutine");
        ...
        {
            yield return null;
            yield return StartCoroutine(BlendLayerWeight(_lastWeight, 0, _blendtreeDuration * 0.25f));
