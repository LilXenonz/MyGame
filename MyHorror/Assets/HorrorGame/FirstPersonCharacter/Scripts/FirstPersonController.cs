...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class FirstPersonController : MonoBehaviour
    {
        ...
        {
            // If a scripted lerp has blocked look, skip MouseLook's normal control.
            if (!m_BlockLook)
            {
                m_MouseLook.LookRotation(transform, m_Camera.transform);
            }
            // else: the DoLerpRotation coroutine is driving transform + camera rotations.
        }
        ...
        // --- Coroutine that performs the smooth rotation ---
        private IEnumerator DoLerpRotation(Transform target, float duration, bool keepLookLocked = false)
        {
            ...
            Quaternion startCamLocal = m_Camera.transform.localRotation;

            // Compute direction from camera to target to reduce parallax
            Vector3 directionToTarget = target.position - m_Camera.transform.position;
            ...
            if (duration <= 0f)
            {
                ...
                m_Camera.transform.localRotation = targetCamLocal;
                m_BlockLook = keepLookLocked;
                if (!m_BlockLook) m_MouseLook.Init(transform, m_Camera.transform);
                m_LerpCoroutine = null;
                yield break;
            }
            ...
            while (elapsed < duration)
            {
                ...
                m_Camera.transform.localRotation = Quaternion.Slerp(startCamLocal, targetCamLocal, eased);
            ...
            // Re-init MouseLook to avoid snapping when control is returned
            if (!m_BlockLook)
            {
                m_MouseLook.Init(transform, m_Camera.transform);
            }
